...
class Ethernet_connection:
    ...
    def __init__(self, hardware_dict=None, stripchart_setup_dict=None, ip_address='0.0.0.0'):
        self.ethernet_commands = {'SyncStart': 1,        # HAVE to use this to 'start' with Ethernet devices
                                  'SlaveIp': 5,
                                  'SyncStop': 6,
    ...
    def pack_command(self, groupid=1,command='Connect',arg0=0,arg1=0,arg2=0,payload=''):
        ...
        packed = struct.pack(pack_format,0x31415926,groupid,self.ethernet_commands[command],arg0,arg1,arg2,payload)
        return packed
    ...
    def set_ascii_eol(self):
        ...
        msg = self.pack_command(groupid=self.new_group_id,command=self.ethernet_commands['Shared'],payload="eol 1")
        self.disc_sock.sendto(msg, ("255.255.255.255", 51235))     # Device's command receiving port
        for message in self.read_messages():
            print(str(message))
    ...
    def send_setup_commands(self, dec=1, deca=1, sample_rate=1000, packet_size=16, encoding='binary'):
        ...
        stripcharts_info = self.stripchart_setup_dict
