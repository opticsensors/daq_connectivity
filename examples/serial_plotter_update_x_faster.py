...
def save_data():
    ...
    try:
        ...
        if x_vals:
            ...
            # df.to_csv(file_path, index=False)  # Commented out data saving
            print(f"\nData would be saved to: {file_path}")
...
def update_plot(frame):
    ...
    # Only update y-axis if data actually exceeds current limits
    try:
        if len(ch_data[0]) > 0:
            # Check only the most recent data points for limit violations
            needs_update = False
            data_max = None
            data_min = None
            ...
            for data in ch_data:
                if len(data) > 0:
                    # Only check recent points to avoid unnecessary recalculations
                    recent_points = min(50, len(data))  # Check only last 50 points
                    recent_data = list(data)[-recent_points:]
                    
                    current_max = max(recent_data)
                    current_min = min(recent_data)
                    
                    # Only flag for update if data actually exceeds current axis limits
                    if current_max > current_ymax or current_min < current_ymin:
                        needs_update = True
                        if data_max is None or current_max > data_max:
                            data_max = current_max
                        if data_min is None or current_min < data_min:
                            data_min = current_min
            
            # Only update if we actually found data that exceeds limits
            if needs_update and data_max is not None and data_min is not None:
                # Expand limits with margin only for the exceeded direction
                if data_max > current_ymax:
                    current_ymax = data_max * yaxis_margin
                if data_min < current_ymin:
                    current_ymin = data_min * yaxis_margin
                    
                print(f"Y-axis updated due to limit breach: [{current_ymin:.2f}, {current_ymax:.2f}]")
